...
def _write_config_yaml(
    ...
    *,
    ...
    comment_source = _normalize_max_turns_config(commented_from or config)
...
def save_user_config(config: Dict[str, Any], *, config_path: Optional[Path] = None):
    ...
    normalized = _normalize_max_turns_config(config)
    raw_existing = _normalize_max_turns_config(load_raw_config(config_path=config_path))
...
def set_config_value(key: str, value: str):
    ...
    # Otherwise it goes to config.yaml. Use the raw user-authored config
    # and save back through the shared raw-preserving writer so this path
    # stays aligned with setup/model/tools saves.
    config_path = get_config_path()
    user_config = load_raw_config()
